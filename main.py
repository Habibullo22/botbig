...
def withdraw_balance(message):
    ...
    try:
        ...
        if user_balances.get(user_id, 0) < amount:
            bot.reply_to(message, "❌ Foydalanuvchida bu miqdor yo‘q.")
            return

        user_balances[user_id] -= amount
        bot.reply_to(message, f"✅ {amount} so‘m foydalanuvchi ({user_id}) balansidan yechildi.")
    except:
        bot.reply_to(message, "❌ Format noto‘g‘ri. To‘g‘ri: /withdraw <id> <miqdor>")
